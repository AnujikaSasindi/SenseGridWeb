import React from 'react'
import Image from 'next/image'

function Footer() {
  return (
    <div className='px-10 bg-[#000] w-full h-auto bottom-0'>
        <div className='grid grid-cols-4 gap-4 px-5 py-5 h-auto'>
            <div className='w-50 h-9/10 px-5 py-5'>
                <img src="./assets/logo.png" alt='' className='sm:w-auto py-10' />
            </div>
            <div className='flex-col justify-between h-9/10 px-5'>
                <div className='py-8'>
                    <h3 className='text-base text-white font-bold'>Type1</h3>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                </div>
                <div className='py-8'>
                <h3 className='text-base text-white font-bold'>Type1</h3>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                </div>
            </div>
            <div className='flex-col justify-between h-9/10 px-5'>
                <div className='py-8'>
                    <h3 className='text-base text-white font-bold'>Type1</h3>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                </div>
                <div className='py-8'>
                <h3 className='text-base text-white font-bold'>Type1</h3>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                </div>
            </div>
            <div className='flex-col justify-between h-9/10'>
                <div className='py-8'>
                    <h3 className='text-base text-white font-bold'>Type1</h3>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                </div>
                <div className='py-8'>
                <h3 className='text-base text-white font-bold'>Type1</h3>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                    <p href='#' className='text-gray-400 hover:text-gray-500 text-light text-normal hover:underline hover:underline-1'>Item1</p>
                </div>
            </div>
        </div>
        <div className='flex justify-between px-5 pb-16 h-auto w-full gap-4'>
            <div className='flex justify-left'>
                <p className='text-normal text-white mr-10'>© 2025, SenseGrid by SLTMobitel.</p>
                <a href='#' className='text-white font-normal underline underline-offset-2 hover:text-gray-400 mr-5'>Terms</a>
                <a href='#' className='text-white font-normal underline underline-offset-2 hover:text-gray-400 mr-5'>Privacy Policy</a>
                <a href='#' className='text-white font-normal underline underline-offset-2 hover:text-gray-400 mr-5'>Cookies</a>
            </div>
            <div className='flex justify-right'>
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 50 50" className="text-white w-6 h-6 mr-10">
                    <path fill="currentColor" d="M 24.402344 9 C 17.800781 9 11.601563 9.5 8.300781 10.199219 C 6.101563 10.699219 4.199219 12.199219 3.800781 14.5 C 3.402344 16.898438 3 20.5 3 25 C 3 29.5 3.398438 33 3.898438 35.5 C 4.300781 37.699219 6.199219 39.300781 8.398438 39.800781 C 11.902344 40.5 17.898438 41 24.5 41 C 31.101563 41 37.097656 40.5 40.597656 39.800781 C 42.800781 39.300781 44.699219 37.800781 45.097656 35.5 C 45.5 33 46 29.402344 46.097656 24.902344 C 46.097656 20.402344 45.597656 16.800781 45.097656 14.300781 C 44.699219 12.101563 42.800781 10.5 40.597656 10 C 37.097656 9.5 31 9 24.402344 9 Z M 24.402344 11 C 31.601563 11 37.398438 11.597656 40.199219 12.097656 C 41.699219 12.5 42.898438 13.5 43.097656 14.800781 C 43.699219 18 44.097656 21.402344 44.097656 24.902344 C 44 29.199219 43.5 32.699219 43.097656 35.199219 C 42.800781 37.097656 40.800781 37.699219 40.199219 37.902344 C 36.597656 38.601563 30.597656 39.097656 24.597656 39.097656 C 18.597656 39.097656 12.5 38.699219 9 37.902344 C 7.5 37.5 6.300781 36.5 6.101563 35.199219 C 5.300781 32.398438 5 28.699219 5 25 C 5 20.398438 5.402344 17 5.800781 14.902344 C 6.101563 13 8.199219 12.398438 8.699219 12.199219 C 12 11.5 18.101563 11 24.402344 11 Z M 19 17 L 19 33 L 33 25 Z M 21 20.402344 L 29 25 L 21 29.597656 Z"></path>
                </svg>
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 50 50" className="text-white w-6 h-6 mr-10">
                    <path fill="currentColor" d="M 11 4 C 7.1456661 4 4 7.1456661 4 11 L 4 39 C 4 42.854334 7.1456661 46 11 46 L 39 46 C 42.854334 46 46 42.854334 46 39 L 46 11 C 46 7.1456661 42.854334 4 39 4 L 11 4 z M 11 6 L 39 6 C 41.773666 6 44 8.2263339 44 11 L 44 39 C 44 41.773666 41.773666 44 39 44 L 11 44 C 8.2263339 44 6 41.773666 6 39 L 6 11 C 6 8.2263339 8.2263339 6 11 6 z M 13.085938 13 L 22.308594 26.103516 L 13 37 L 15.5 37 L 23.4375 27.707031 L 29.976562 37 L 37.914062 37 L 27.789062 22.613281 L 36 13 L 33.5 13 L 26.660156 21.009766 L 21.023438 13 L 13.085938 13 z M 16.914062 15 L 19.978516 15 L 34.085938 35 L 31.021484 35 L 16.914062 15 z"></path>
                </svg>
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 50 50" className="text-white w-6 h-6 mr-10">
                    <path fill="currentColor" d="M 11.300781 2 C 6.1645496 2 2 6.1645496 2 11.300781 L 2 38.800781 C 2 43.937013 6.1645496 48.099609 11.300781 48.099609 L 38.800781 48.099609 C 43.937013 48.099609 48.099609 43.937013 48.099609 38.800781 L 48.099609 11.289062 L 48.099609 11.277344 C 47.988214 6.1531405 43.848929 2 38.800781 2 L 11.300781 2 z M 11.300781 4 L 38.800781 4 C 42.752633 4 46.011005 7.2464683 46.099609 11.322266 L 46.099609 38.800781 C 46.099609 42.864549 42.864549 46.099609 38.800781 46.099609 L 33 46.099609 L 33 29 L 37.847656 29 L 39.179688 21 L 33 21 L 33 19 C 33 18.45 33.05476 18.405705 33.251953 18.279297 C 33.44915 18.152889 34.029365 18 35 18 L 39 18 L 39 11.271484 L 38.306641 11.048828 C 38.306641 11.048828 35.129885 10 32 10 C 29.096296 10 26.957792 10.953443 25.679688 12.632812 C 24.401582 14.312183 24 16.536525 24 19 L 24 21 L 21 21 L 21 29 L 24 29 L 24 46.099609 L 11.300781 46.099609 C 7.2370133 46.099609 4 42.864549 4 38.800781 L 4 11.300781 C 4 7.2370133 7.2370133 4 11.300781 4 z M 32 12 C 34.168683 12 36.174546 12.537635 37 12.773438 L 37 16 L 35 16 C 33.870635 16 32.949678 16.09711 32.171875 16.595703 C 31.394072 17.094295 31 18.05 31 19 L 31 23 L 36.820312 23 L 36.152344 27 L 31 27 L 31 46.099609 L 26 46.099609 L 26 27 L 23 27 L 23 23 L 26 23 L 26 19 C 26 16.763475 26.399589 14.98938 27.271484 13.84375 C 28.14338 12.69812 29.503704 12 32 12 z"></path>
                </svg>
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 50 50" className="text-white w-6 h-6 mr-10">
                    <path fill="currentColor" d="M 16 3 C 8.8324839 3 3 8.8324839 3 16 L 3 34 C 3 41.167516 8.8324839 47 16 47 L 34 47 C 41.167516 47 47 41.167516 47 34 L 47 16 C 47 8.8324839 41.167516 3 34 3 L 16 3 z M 16 5 L 34 5 C 40.086484 5 45 9.9135161 45 16 L 45 34 C 45 40.086484 40.086484 45 34 45 L 16 45 C 9.9135161 45 5 40.086484 5 34 L 5 16 C 5 9.9135161 9.9135161 5 16 5 z M 37 11 A 2 2 0 0 0 35 13 A 2 2 0 0 0 37 15 A 2 2 0 0 0 39 13 A 2 2 0 0 0 37 11 z M 25 14 C 18.936712 14 14 18.936712 14 25 C 14 31.063288 18.936712 36 25 36 C 31.063288 36 36 31.063288 36 25 C 36 18.936712 31.063288 14 25 14 z M 25 16 C 29.982407 16 34 20.017593 34 25 C 34 29.982407 29.982407 34 25 34 C 20.017593 34 16 29.982407 16 25 C 16 20.017593 20.017593 16 25 16 z"></path>
                </svg>
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 50 50" className="text-white w-6 h-6 mr-10">
                    <path fill="currentColor" d="M 9 4 C 6.2504839 4 4 6.2504839 4 9 L 4 41 C 4 43.749516 6.2504839 46 9 46 L 41 46 C 43.749516 46 46 43.749516 46 41 L 46 9 C 46 6.2504839 43.749516 4 41 4 L 9 4 z M 9 6 L 41 6 C 42.668484 6 44 7.3315161 44 9 L 44 41 C 44 42.668484 42.668484 44 41 44 L 9 44 C 7.3315161 44 6 42.668484 6 41 L 6 9 C 6 7.3315161 7.3315161 6 9 6 z M 14 11.011719 C 12.904779 11.011719 11.919219 11.339079 11.189453 11.953125 C 10.459687 12.567171 10.011719 13.484511 10.011719 14.466797 C 10.011719 16.333977 11.631285 17.789609 13.691406 17.933594 A 0.98809878 0.98809878 0 0 0 13.695312 17.935547 A 0.98809878 0.98809878 0 0 0 14 17.988281 C 16.27301 17.988281 17.988281 16.396083 17.988281 14.466797 A 0.98809878 0.98809878 0 0 0 17.986328 14.414062 C 17.884577 12.513831 16.190443 11.011719 14 11.011719 z M 14 12.988281 C 15.392231 12.988281 15.94197 13.610038 16.001953 14.492188 C 15.989803 15.348434 15.460091 16.011719 14 16.011719 C 12.614594 16.011719 11.988281 15.302225 11.988281 14.466797 C 11.988281 14.049083 12.140703 13.734298 12.460938 13.464844 C 12.78117 13.19539 13.295221 12.988281 14 12.988281 z M 11 19 A 1.0001 1.0001 0 0 0 10 20 L 10 39 A 1.0001 1.0001 0 0 0 11 40 L 17 40 A 1.0001 1.0001 0 0 0 18 39 L 18 33.134766 L 18 20 A 1.0001 1.0001 0 0 0 17 19 L 11 19 z M 20 19 A 1.0001 1.0001 0 0 0 19 20 L 19 39 A 1.0001 1.0001 0 0 0 20 40 L 26 40 A 1.0001 1.0001 0 0 0 27 39 L 27 29 C 27 28.170333 27.226394 27.345035 27.625 26.804688 C 28.023606 26.264339 28.526466 25.940057 29.482422 25.957031 C 30.468166 25.973981 30.989999 26.311669 31.384766 26.841797 C 31.779532 27.371924 32 28.166667 32 29 L 32 39 A 1.0001 1.0001 0 0 0 33 40 L 39 40 A 1.0001 1.0001 0 0 0 40 39 L 40 28.261719 C 40 25.300181 39.122788 22.95433 37.619141 21.367188 C 36.115493 19.780044 34.024172 19 31.8125 19 C 29.710483 19 28.110853 19.704889 27 20.423828 L 27 20 A 1.0001 1.0001 0 0 0 26 19 L 20 19 z M 12 21 L 16 21 L 16 33.134766 L 16 38 L 12 38 L 12 21 z M 21 21 L 25 21 L 25 22.560547 A 1.0001 1.0001 0 0 0 26.798828 23.162109 C 26.798828 23.162109 28.369194 21 31.8125 21 C 33.565828 21 35.069366 21.582581 36.167969 22.742188 C 37.266572 23.901794 38 25.688257 38 28.261719 L 38 38 L 34 38 L 34 29 C 34 27.833333 33.720468 26.627107 32.990234 25.646484 C 32.260001 24.665862 31.031834 23.983076 29.517578 23.957031 C 27.995534 23.930001 26.747519 24.626988 26.015625 25.619141 C 25.283731 26.611293 25 27.829667 25 29 L 25 38 L 21 38 L 21 21 z"></path>
                </svg>
            </div>
        </div>

    </div>
  )
}

export default Footer